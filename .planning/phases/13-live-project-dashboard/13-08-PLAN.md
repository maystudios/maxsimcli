...
<task type="auto">
  ...
  <action>
  Run full build and fix any integration issues:
  ...
  8. Polish visual consistency:
     - Verify all components use theme CSS variables (not hardcoded colors)
     - Ensure font-mono and font-sans are applied correctly per component
     - Check spacing grid is consistent (p-4, p-6, gap-3, gap-4 patterns)
     - Ensure accent-glow effects on current phase and interactive elements
  9. Add a loading spinner component for initial data fetch states
  10. Verify `pnpm install && nx build dashboard` passes from a clean state
  </action>
  <verify>
...
<task type="checkpoint:human-verify" gate="blocking">
  ...
  8. Verify the dark theme looks consistent — no bright white areas, all text readable
  </how-to-verify>
