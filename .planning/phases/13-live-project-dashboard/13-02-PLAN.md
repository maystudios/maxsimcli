...
must_haves:
  truths:
    - "Custom server starts and hosts both Next.js and WebSocket on the same port"
    - "WebSocket server handles upgrade requests at /api/ws without interfering with Next.js HMR at /_next/webpack-hmr"
    - "chokidar watches .planning/ directory and broadcasts debounced file change events to all connected WebSocket clients"
    - "React WebSocket provider auto-reconnects on disconnect and exposes lastChange timestamp for data refetching"
    - "Server auto-detects free port starting from 3333 and auto-opens browser"
  artifacts:
