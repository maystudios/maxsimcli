...
must_haves:
  truths:
    ...
    - "A developer running 'next build' in packages/dashboard gets a successful build with zero errors"
    - "Opening the dashboard in a browser shows a dark-themed page with Swiss Style Design colors"
    - "Tailwind v4 CSS-first config applies dark theme by default"
    - "Aceternity UI components can use cn() utility for conditional class merging"
  artifacts:
...
<task type="auto">
  ...
  **IMPORTANT: Ignore aceternity-ui SKILL.md Tailwind patterns — they document Tailwind v3 (framer-motion, @tailwind directives, tailwind.config.ts). Use Tailwind v4 @import and CSS-first config from RESEARCH.md only. Use `motion` (not `framer-motion`) and `@import "tailwindcss"` (not `@tailwind base/components/utilities`).**
