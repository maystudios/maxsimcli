...
2. In `packages/cli/tests/e2e/install.test.ts`, line 16 has `it('installs exactly 13 agent .md files'` — this is ALREADY correct (it was fixed in Phase 27). Verify this is the case. No change needed if it already says 13.
  </action>
  <verify>
    <automated>grep -c "32 commands" .planning/ROADMAP.md | grep -q "1" && grep "13 agent" packages/cli/tests/e2e/install.test.ts | grep -q "13" && echo "PASS" || echo "FAIL"</automated>
  </verify>
