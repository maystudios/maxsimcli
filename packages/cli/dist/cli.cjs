...
function cmdSkillContext(cwd, skillName, raw) {
	...
	if (phaseDir) {
		const absPhaseDir = node_path.default.isAbsolute(phaseDir) ? phaseDir : node_path.default.join(cwd, phaseDir);
		try {
			const files = node_fs.default.readdirSync(absPhaseDir);
			for (const f of files) if (isPlanFile(f)) artifacts.plan = node_path.default.join(phaseDir, f);
			else if (isSummaryFile(f)) artifacts.summary = node_path.default.join(phaseDir, f);
			else if (f.endsWith("-RESEARCH.md") || f === "RESEARCH.md") artifacts.research = node_path.default.join(phaseDir, f);
			else if (f.endsWith("-CONTEXT.md") || f === "CONTEXT.md") artifacts.context = node_path.default.join(phaseDir, f);
			else if (f.endsWith("-VERIFICATION.md") || f === "VERIFICATION.md") artifacts.verification = node_path.default.join(phaseDir, f);
		} catch {}
	}
	const totalPlans = stateContent ? stateExtractField(stateContent, "Total Plans in Phase") : null;
	const position = currentPlan && totalPlans ? `Plan ${currentPlan} of ${totalPlans}` : currentPlan ? `Plan ${currentPlan}` : null;
	output({
		skill_name: skillName,
