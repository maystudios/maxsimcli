...
*/
const adapterMap = { claude: claudeAdapter };
/**
...
*/
function finishInstall(settingsPath, settings, statuslineCommand, shouldInstallStatusline, _runtime = "claude", _isGlobal = true) {
...
*/
function writeManifest(configDir, _runtime = "claude") {
...
function copyWithPathReplacement(srcDir, destDir, pathPrefix, runtime, explicitConfigDir) {
	...
	for (const entry of entries) {
		...
		if (entry.isDirectory()) copyWithPathReplacement(srcPath, destPath, pathPrefix, runtime, explicitConfigDir);
...
async function promptLocation() {
	...
	const globalPath = getGlobalDir("claude", explicitConfigDir).replace(node_os.homedir(), "~");
