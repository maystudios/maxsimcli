...
export async function cmdStart(
  ...
  if (existingPort) {
    ...
    output({ started: true, url, already_running: true, port: existingPort }, raw, url);
    return;
  }
  ...
  if (!serverPath) {
    error('Dashboard server not found. Run `npx maxsimcli` to install first.');
  }
  ...
  if (!pid) {
    error('Failed to spawn dashboard process.');
  }
  ...
  if (url) {
    if (!options.noBrowser) openBrowser(url);
    output({ started: true, url, already_running: false, pid }, raw, url);
  } else {
