...
```bash
PHASE_INFO=$(node ~/.claude/maxsim/bin/maxsim-tools.cjs find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node ~/.claude/maxsim/bin/maxsim-tools.cjs roadmap get-phase "${PHASE}")
...
```bash
PHASE_DIR=$(node ~/.claude/maxsim/bin/maxsim-tools.cjs find-phase "${PHASE}" --raw)
```
