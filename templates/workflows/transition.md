...
**Delegate ROADMAP.md and STATE.md updates to maxsim-tools:**

```bash
TRANSITION=$(node ~/.claude/maxsim/bin/maxsim-tools.cjs phase complete "${current_phase}")
```
...
**Note:** Basic position updates (Current Phase, Status, Current Plan, Last Activity) were already handled by `maxsim-tools phase complete` in the update_roadmap_and_state step.

Verify the updates are correct by reading STATE.md. If the progress bar needs updating, use:

```bash
PROGRESS=$(node ~/.claude/maxsim/bin/maxsim-tools.cjs progress bar --raw)
```
...
**Use the transition result from `maxsim-tools phase complete`:**
...
Exit skill and invoke SlashCommand("/maxsim:plan-phase [X+1] --auto")
...
Exit skill and invoke SlashCommand("/maxsim:discuss-phase [X+1] --auto")
...
`/maxsim:discuss-phase [X+1]` — gather context and clarify approach
...
**Also available:**
- `/maxsim:plan-phase [X+1]` — skip discussion, plan directly
- `/maxsim:research-phase [X+1]` — investigate unknowns
...
**Also available:**
- `/maxsim:discuss-phase [X+1]` — revisit context
- `/maxsim:research-phase [X+1]` — investigate unknowns
...
```bash
node ~/.claude/maxsim/bin/maxsim-tools.cjs config-set workflow.auto_advance false
```
...
Exit skill and invoke SlashCommand("/maxsim:complete-milestone {version}")
